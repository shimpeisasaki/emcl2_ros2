...
void GnssUtil::gnssReset(double alpha, double alpha_th, std::vector<emcl2::Particle> & particles)
{
    ...
    {
        particles[i].p_.x_ = gnss_position_[0] + pfRanGaussian(sqrt(gnss_sigma_mx_(0, 0) * 3.0));
        particles[i].p_.y_ = gnss_position_[1] + pfRanGaussian(sqrt(gnss_sigma_mx_(1, 1) * 3.0));
